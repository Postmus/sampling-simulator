...
test("waits for evaluation before collecting a user-defined line", async ({ page }) => {
  ...
  await expect(page.getByText("ŷ = 70 + 0x", { exact: true })).toBeVisible();
  await expect(page.getByText("mean of y = 70.0", { exact: true })).toBeVisible();
  ...
  const scatterYAxis = await page.locator(".regression-axis-title").filter({ hasText: "Exam score" }).boundingBox();
  ...
  await expect(page.locator(".residual-square")).toHaveCount(9);
  await expect(page.locator(".moving-squared-error")).toHaveCount(9);
  await expect(page.locator(".moving-residual-token")).toHaveCount(9);
  ...
  await expect(page.getByText("Negative means below the line; positive means above the line.")).toBeVisible();
});
